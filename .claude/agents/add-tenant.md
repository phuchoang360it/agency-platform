...
## Steps to follow
1. Read `src/tenants/__fixture__/tenant.config.ts` and `src/tenants/__fixture__/seed.ts` as reference
2. Create `src/tenants/<slug>/tenant.config.ts` with the client's details
3. Create `src/tenants/<slug>/seed.ts` with placeholder content
